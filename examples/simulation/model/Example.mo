model Example
  ...
  Deltares.ChannelFlow.SimpleRouting.Storage.Storage storage(Q_release = PI_control, V(start=V_storage_init, nominal=4e5)) annotation(Placement(visible = true, transformation(origin = {0, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  ...
  PI_contoller PI(measured = storage.V, setpoint = storage_V_SP, Kp = 1.0, Ki = 1e-6);
  // Initial States
  parameter Modelica.SIunits.Volume V_storage_init;
  // Inputs
  input Modelica.SIunits.VolumeFlowRate PI_control(fixed = true);
  ...
  output Modelica.SIunits.VolumeFlowRate Q_release = PI_control;
equation
